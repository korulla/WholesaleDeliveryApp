  ...
  const db = firebase.firestore();
          ...
          function placeOrder() {
              ...
              for (let i = 0; i < cartItems.length; i++) {
                  ...
                  const quantity = parseInt(itemText.split("x")[1].trim());
                  const productId = cartItems[i].dataset.productId; // Get the product ID from the data attribute
                  order.products.push({ productId, name, quantity }); // Include the product ID in the order
              }
